...
class ALTrainer:
    ...
    def __init__(
        self, model, lr, train_loader, valid_loader, test_loader, save_dir, label_num=None, double=False, class_num=None, f1=False
    ):
    ...
    def run(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            for inputs, labels in self.train_loader:
                ...
                losses["emb bridge"].append(self.model.embedding.loss_b)
                losses["emb associated"].append(self.model.embedding.loss_d)
                losses["lstm1 bridge"].append(self.model.layer_1.loss_b)
                losses["lstm1 associated"].append(self.model.layer_1.loss_d)
                losses["lstm2 bridge"].append(self.model.layer_2.loss_b)
                losses["lstm2 associated"].append(self.model.layer_2.loss_d)
                self.opt.step()
                ...
                with torch.no_grad():
                    ...
                    if self.label_num == 2:
                        ...
                        total_acc += (predicted_label.argmax(-1) ==
                                      labels.to(torch.float).argmax(-1)).sum().item()
                    if self.f1 == True:
                        pred = (predicted_label>0.5).float().tolist()
                        f1_pred = f1_pred + pred
                        label = labels.tolist()
                        f1_label = f1_label + label
                    total_count += labels.size(0)
            ...
            for inputs, labels in self.valid_loader:
                ...
                with torch.no_grad():
                    ...
                    if self.label_num == 2:
                        ...
                        val_label = val_label + labels.to(torch.float).argmax(-1).tolist()
                        val_pred = val_pred + predicted_label.argmax(-1).tolist()
                        if self.f1:
                            f1_pred_val = f1_pred_val + (predicted_label>0.5).float().tolist()
                            f1_label_val = f1_label_val + labels.tolist()
            ...
            epoch_val_acc = val_acc/val_count
            if self.f1:
                epoch_train_acc = f1_score(f1_pred, f1_label, average='macro')
                epoch_val_acc = f1_score(f1_pred_val, f1_label_val, average='macro')
    ...
    def eval(self):
        ...
        for inputs, labels in self.test_loader:
            ...
            with torch.no_grad():
                ...
                all_preds = all_preds + predicted_label.argmax(-1).tolist()
                all_labels = all_labels + labels.to(torch.float).argmax(-1).tolist()
                if self.f1:
                    f1_pred = f1_pred + (predicted_label>0.5).tolist()
                    f1_label = f1_label + labels.tolist()
    ...
    def pred(self):
        ...
        for inputs, labels in self.test_loader:
            ...
            left = self.model.embedding.f(inputs)
            output, hidden = self.model.layer_1.f(left)
            output, hidden = self.model.layer_2.f(output, hidden)
            left = output[:, -1, :]
            right = self.model.layer_2.bx(left)
            right = self.model.layer_2.dy(right)
            right = self.model.layer_1.dy(right)
        ...
        print(f'write back to {tsv_name} finished')
