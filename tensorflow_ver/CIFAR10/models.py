...
def autoencoders(y_placeholder, neuron_num, num_of_ae, inv_learning_rate, y_function):
    ...
    for i in range(num_of_ae - 1):
        y_inv = tf.layers.dense(y_list[num_of_ae - 1 - i], neuron_num, y_function, name='y_inv_%d' % (num_of_ae - 1 - i),
                                kernel_initializer=tf.keras.initializers.he_normal())
        i_weights.append(tf.get_default_graph().get_tensor_by_name(
            os.path.split(y_inv.name)[0] + '/kernel:0'))
        i_biases.append(tf.get_default_graph().get_tensor_by_name(
            os.path.split(y_inv.name)[0] + '/bias:0'))

        rev_loss = tf.losses.mean_squared_error(
            predictions=y_inv, labels=tf.stop_gradient(y_list[num_of_ae - 2 - i]))
        rev_train_steps.append(tf.train.AdamOptimizer(
            inv_learning_rate).minimize(rev_loss))
    ...
    rev_loss = tf.losses.mean_squared_error(
        predictions=y_inv, labels=y_placeholder)
    global_step = tf.train.get_or_create_global_step()
    rev_train_steps.append(tf.train.AdamOptimizer(
        inv_learning_rate).minimize(rev_loss, global_step=global_step))
...
def al_loss(inputs, y_list, index, name_scope, train_steps, update_opss, learning_rate, bridge_function, neuron_num, regularizer=None):
    ...
    if type(name_scope) == list:
        ...
        for i in range(1, len(name_scope)):
            ...
            update_ops = tf.get_collection(
                tf.GraphKeys.UPDATE_OPS, name_scope[i])
            update_opss.append(update_ops)
    else:
    ...
    loss = tf.losses.mean_squared_error(
        predictions=dense, labels=tf.stop_gradient(y_list[index])) + l2_loss
...
def resBlock(inputs, out_channels, strides, regularizer, is_training, f_function):
    conv1 = conv_layer_bn(inputs, out_channels, regularizer,
                          is_training, f_function, strides)

    conv2 = conv_layer_bn(conv1, out_channels, regularizer,
                          is_training, f_function, 1)
...
def ResNet(num_blocks, regulariztion_scale, neuron_num, X_placeholder, y_placeholder, learning_rate, inv_learning_rate, f_function, is_training):
    ...
    for stride in stride_list:
        ...
        with tf.name_scope('Layer_' + str(layer_count)):
            for stride in strides:
                resblock = resBlock(
                    resblock, num_filter, stride, regularizer, is_training, f_function)
        num_filter *= 2
        layer_count += 1
    ...
    with tf.name_scope('Dense_0'):
        net = tf.layers.dense(flat, 5*neuron_num, kernel_initializer=tf.initializers.he_normal(),
                              kernel_regularizer=regularizer)
        net = f_function(tf.layers.batch_normalization(
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=y_placeholder, logits=net)) + l2_loss
    global_step = tf.train.get_or_create_global_step()
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    train_steps = tf.train.AdamOptimizer(
...
def ResNet_AL(num_blocks, regulariztion_scale, neuron_num, X_placeholder, y_placeholder, learning_rate, inv_learning_rate, acti_functions, is_training):
    ...
    rev_train_steps, global_step, target_list, i_weights, i_biases = \
        autoencoders(y_placeholder, neuron_num, num_of_ae,
                     inv_learning_rate, y_function)
    ...
    all_group_train_steps = tf.group(
        train_steps + rev_train_steps + update_opss)
    # inference
...
def MLP(X_placeholder, y_placeholder, learning_rate, inv_learning_rate, is_training):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=y_placeholder, logits=net))
    global_step = tf.train.get_or_create_global_step()
    train_steps = tf.train.AdamOptimizer(
        learning_rate).minimize(loss, global_step)
    return net, train_steps, global_step, neuron_num
...
def MLP_AL(X_placeholder, y_placeholder, learning_rate, inv_learning_rate, is_training):
    ...
    for i in range(num_of_ae):
        ...
        dense = al_loss(net, target_list, i, 'Layer_' + str(i+1), train_steps,
                        update_opss, learning_rate, bridge_function, neuron_num)
...
def CNN(X_placeholder, y_placeholder, learning_rate, inv_learning_rate, is_training):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=y_placeholder, logits=net)) + tf.losses.get_regularization_loss()
    global_step = tf.train.get_or_create_global_step()
    train_steps = tf.train.AdamOptimizer(
        learning_rate).minimize(loss, global_step)
    return net, train_steps, global_step, neuron_num
...
def CNN_AL(X_placeholder, y_placeholder, learning_rate, inv_learning_rate, is_training):
    ...
    for i in range(hidden_layer_count - 1):
        ...
        if (i % 2) == 1:
            al_loss(net, target_list, target_index, name_scope, train_steps,
                    update_opss, learning_rate, bridge_function, neuron_num)
    ...
    dense = al_loss(net, target_list, target_index, name_scope, train_steps,
                    update_opss, learning_rate, bridge_function, neuron_num)
    net = tf.stop_gradient(net)
    target_index += 1
...
def VGG(X_placeholder, y_placeholder, learning_rate, inv_learning_rate, is_training):
    ...
    # model
    net = conv_layer_bn(X_placeholder, 128, regularizer,
                        is_training, f_function)
    ...
    l2_loss = tf.losses.get_regularization_loss()
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        logits=net, labels=y_placeholder)) + l2_loss
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    global_step = tf.train.get_or_create_global_step()
...
def VGG_AL(X_placeholder, y_placeholder, learning_rate, inv_learning_rate, is_training):
    ...
    net = conv_layer_bn(X_placeholder, 128, regularizer, is_training,
                        f_function, name_scope=name_scope, is_batchnorm=False)

    name_scope = 'Layer_1'
    net = conv_layer_bn(net, 256, regularizer, is_training,
                        f_function, name_scope=name_scope, is_batchnorm=False)
    net = tf.layers.max_pooling2d(net, pool_size=2, strides=2)
    al_loss(net, target_list, 0, ['Layer_0', 'Layer_1'], train_steps,
            update_opss, learning_rate, bridge_function, neuron_num, regularizer)
    net = tf.stop_gradient(net)
    ...
    name_scope = 'Layer_3'
    net = conv_layer_bn(net, 512, regularizer, is_training,
                        f_function, name_scope=name_scope, is_batchnorm=False)
    net = tf.layers.max_pooling2d(net, pool_size=2, strides=2)
    al_loss(net, target_list, 1, ['Layer_2', 'Layer_3'], train_steps,
            update_opss, learning_rate, bridge_function, neuron_num, regularizer)
    net = tf.stop_gradient(net)
    ...
    net = conv_layer_bn(net, 512, regularizer, is_training,
                        f_function, name_scope=name_scope, is_batchnorm=False)
    net = tf.layers.max_pooling2d(net, pool_size=2, strides=2)
    al_loss(net, target_list, 2, ['Layer_4', 'Layer_5'], train_steps,
            update_opss, learning_rate, bridge_function, neuron_num, regularizer)
    net = tf.stop_gradient(net)
    ...
    net = tf.layers.dense(net, 1024, f_function, regularizer,
                          kernel_initializer=tf.keras.initializers.he_normal(), name=name_scope)
    dense = al_loss(net, target_list, 3, ['Layer_6', 'Layer_7'], train_steps,
                    update_opss, learning_rate, bridge_function, neuron_num, regularizer)
