...
class NMT(nn.Module):
    def __init__(self, args, tkr):
        ...
        self.src_embed = nn.Embedding(25000, args.embed_size)
        self.tgt_embed = nn.Embedding(25000, args.embed_size)

        self.encoder_lstm = nn.LSTM(args.embed_size, args.hidden_size, bidirectional=True, dropout=args.dropout, batch_first=True)
        self.decoder_lstm = nn.LSTMCell(args.embed_size + args.hidden_size, args.hidden_size, batch_first=True)
        ...
        self.readout = nn.Linear(args.hidden_size, 25000, bias=False)
    ...
    def forward(self, src_sents, src_sents_len, tgt_words):
        src_encodings, init_ctx_vec = self.encode(src_sents)
    ...
    def encode(self, src_sents):
        """
        :param src_sents: (batch_size, src_sent_len), sorted by the length of the source
    ...
    def translate(self, src_sents, beam_size=None, to_word=True):
        ...
        src_sents_var = to_input_variable(src_sents, self.tkr, cuda=args.cuda, is_test=True)
...
def to_input_variable(sents, tkr, cuda=False, is_test=False):
    """
