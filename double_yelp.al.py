...
train_label = multi_doubleclass_process([b['label'] for b in news_train], class_num)

test_text = [b['text'] for b in new_test]
test_label = multi_doubleclass_process([b['label'] for b in new_test], class_num)
...
if args.pretrain_emb == 'none': 
    emb = EmbeddingAL((args.vocab_size, class_num*2), (args.word_emb,
...
T = ALTrainer(model, args.lr, train_loader=train_loader,
              valid_loader=valid_loader, test_loader=test_loader, save_dir=args.save_dir, double=True, class_num=5)
